...
import type { Threat, ThreatClassification } from '@/entities/threat'
import type { InterceptorReport, PlatformView } from '@/entities/platform'
import type { FlyingInterceptor } from '@/entities/interceptor'
import type { Burst, EngagementReport, FeedEvent, ThreatDestroyed } from '@/entities/engagement'

const REMOVE_GRACE_MS = 3_000
const FLUSH_MS = 150

export function useTelemetry(url: string = NATS_WS_URL) {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      void (async () => {
        for await (const message of connection.subscribe(MAP_THREATS_SUBJECT)) {
      ...
      void (async () => {
        for await (const message of connection.subscribe(PLATFORM_REPORT_SUBJECT)) {
